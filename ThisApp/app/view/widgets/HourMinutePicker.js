...
Date:13/11/2013
**/
Ext.define("ThisApp.view.widgets.HourMinutePicker", {
    extend: 'Ext.picker.Picker',
    xtype: 'hourMinutePicker',
	...
	config: {
            ...
            */
            hourText: (Ext.os.deviceType.toLowerCase() == "phone") ? 'Hour' : 'Hour',

            /**
             * @cfg {String} minuteText
             * The label to show for the minute column. Defaults to 'Minute'.
             */
            minuteText: (Ext.os.deviceType.toLowerCase() == "phone") ? 'Minute' : 'Minute',
        ...
        getValue: function() {
            ...
            //ThisApp.util.GlobalUtil.logMessage("this.getStore=="+this.getStore);

            /*changes by P for timepicker field Start*/
            if(this.getSlotByName('year') == null ||this.getSlotByName('month') == null ||this.getSlotByName('day') == null){
               //ThisApp.util.GlobalUtil.logMessage("At line 164 this.getSlotByName('year')==="+this.getSlotByName('year'));
               if( this.getSlotByName('hour')!= null && this.getSlotByName('minute')!= null){            /*Added by Sandip for null End Time allowing on 4 dec 2013*/
                   ...
                   hourval = (isNaN(hour) || hour==null) ? this.getSlotByName('hour').getStore().getAt(0).get(this.getSlotByName('hour').getValueField()) : hour,
                   minuteval = (isNaN(minute) || minute==null) ? this.getSlotByName('minute').getStore().getAt(0).get(this.getSlotByName('minute').getValueField()) : minute;
               }
            }else{
                //ThisApp.util.GlobalUtil.logMessage("year ,day month is not null");
                if( this.getSlotByName('hour')!= null && this.getSlotByName('minute')!= null){     /*Added by Sandip for null End Time allowing on 4 dec 2013*/
                    var yearval = (isNaN(values.year) || values.year==null) ?  this.getSlotByName('year').getStore().getAt(0).get(this.getSlotByName('year').getValueField()) : values.year,
                    monthval = (isNaN(values.month) || values.month==null) ? this.getSlotByName('month').getStore().getAt(0).get(this.getSlotByName('month').getValueField())-1 : (values.month - 1),
                    dayval = (isNaN(day) ||day==null) ? this.getSlotByName('day').getStore().getAt(0).get(this.getSlotByName('day').getValueField()) : day,
                    hourval = (isNaN(hour) || hour==null) ? this.getSlotByName('hour').getStore().getAt(0).get(this.getSlotByName('hour').getValueField()) : hour,
                    minuteval = (isNaN(minute) || minute==null) ? this.getSlotByName('minute').getStore().getAt(0).get(this.getSlotByName('minute').getValueField()) : minute;
                }
            }
        ...
        createSlots: function() {
            ...
            var hourLimit;
            if (this.getMaxHours() == 0)
                 hourLimit =  (this.getAmpm()) ? 12 : 23
             else
                 hourLimit = this.getMaxHours()
           ...
           // ThisApp.util.GlobalUtil.logMessage("slots At 373==",slots);
            me.setSlots(slots);
        },
